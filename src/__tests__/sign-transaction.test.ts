...
describe('Testing for signing of transactions', () => {
    ...
    const { transaction, publicKey, privateKey } = createRootTransaction(0, ledgerTimestamp(), 'homer', 'My Test Ledger');

    it('should throw a verification error due to key mistmatch on root transaction', () => {
        const { transactions: lTrans } = mockChain([25.99, 11.0, 54.11, -44.43], 'homer');
    ...
    it('should be able to sign transactions', () => {
        const { transactions: lTrans, publicKey: pubKey, privateKey: privKey } = mockChain(
            [25.99, 11.0, 54.11, -44.43],
            'homer'
        );

        lTrans.push(signTransaction({
            value: 7.99,
