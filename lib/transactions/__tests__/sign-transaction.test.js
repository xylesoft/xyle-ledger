...
const verify_ledger_1 = __importDefault(require("../../ledgers/verify-ledger"));
...
describe('Testing for signing of transactions', () => {
    ...
    const { publicKey, privateKey } = create_root_transaction_1.default(0, ledger_timestamp_1.default(), 'homer', 'My Test Ledger');
    it('should throw a verification error due to key mistmatch on root transaction', () => {
        const { transactions: lTrans } = helper_1.mockLedger([25.99, 11, 54.11, -44.43], 'homer');
        expect(() => verify_ledger_1.default(lTrans, publicKey, privateKey, 'homer')).toThrowError(/Signature error:.+/);
    });
    it('should be able to sign transactions', () => {
        const { transactions: lTrans, publicKey: pubKey, privateKey: privKey } = helper_1.mockLedger([25.99, 11, 54.11, -44.43], 'homer');
        ...
        expect(verify_ledger_1.default(lTrans, pubKey, privKey, 'homer')).toBeTruthy();
    });
});
